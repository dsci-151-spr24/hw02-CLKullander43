...
**Mathematics And Computer Science, Military Technologies, Botany, Soil Science, and Educational Administration And Supervision all have an equally low unemployment rate of 0%**
...
**We often use median rather than mean because it is where someone in the middle would actually land, and it isn't influenced by outliers in data. Essentially, it is less likely to be misleading than the mean is.** 
...
**I went with 5000 because it is much easier to read, and I think it still conveys the data in a meaningful way.**
...
**The median is useful for describing the histogram. It shows a large spike at around the $35000 mark. Also, the quartile marks are useful, as they show us how much of the data is compressed into the relatively small range of $33000-$45000. The min and max show the surpring observations of $22000 and $110000 respectively.**
...
**Engineering has the highest typical median income, with the average median income at over $57000.**
...
**Interdisciplinary is the lowest by far, with only one observation.**
...
ggplot(data = college_recent_grads, 
       mapping = aes(x = median, y = sharewomen, color = major_type)) +
  geom_point()

```

**On average, there are fewer women in stem fields. More women also tend to do majors that pay less. The general trend is that as the number of women decreases, the pay increases and it is more likely to be a stem field.**
...
**How do unemployment rates and non-college job rates compare between stem and non-stem fields?**

```{r stem-jobs}
ggplot(data = college_recent_grads, 
       mapping = aes(x = unemployment_rate, y = non_college_jobs, color = major_type)) +
  geom_point() +
  ylim(0, 100000)

```

**The unemployment rate is surpisingly similar between fields. However, stem fields have a much lower rate of non-college jobs.**
